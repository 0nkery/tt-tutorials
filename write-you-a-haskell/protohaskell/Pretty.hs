...
instance Pretty S.Expr where
  ppr p ex = case ex of
    ...
    S.ELit (S.LitChar c) -> quotes $ char c
...
instance Pretty S.BindGroup where
  ppr p (S.BindGroup f xs ty wh) =
    ...
    where
      prefix = (pp f <+>)
      -- top level Matches use (=) instead of (->)
...
instance Pretty S.Decl where
  ppr p decl = case decl of
    ...
    S.DataDecl conId args cons ->
      "data" <+> ppr p conId <+> spaced p args <+>
      "where" $+$ nest 2 (vcat (fmap pp cons))

    S.ClassDecl preds conId args defs ->
      "class" <+> ppcontext preds <+> ppr p conId <+> spaced p args <+>
      "where" $+$ nest 2 (vcat (fmap pp defs))

    S.InstDecl preds conId ty defs ->
      "instance" <+> ppcontext preds <+> ppr p conId <+> pp ty <+>
      "where" $+$ nest 2 (vcat (fmap pp defs))
