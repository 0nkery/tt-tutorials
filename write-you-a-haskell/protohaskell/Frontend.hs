...
data Literal
  = LitInt Int         -- 1
  | LitChar Char       -- 'a'
  | ListString [Word8] -- A primitive C-style string, type Addr#
  deriving (Eq, Show)
...
groupDecls decls = FunDecl <$> groupBindings (concatMap fgroup decls)
...
composeM :: (Applicative m, Monad m) => (Expr -> m Expr) -> (Expr -> m Expr) -> (Expr -> m Expr)
...
instance AllVars Match where
  allVars (Match pats rhs) = allVars rhs
...
instance FreeVars Match where
  freeVars (Match pats rhs) = freeVars rhs Set.\\ Set.unions (fmap allVars pats)
