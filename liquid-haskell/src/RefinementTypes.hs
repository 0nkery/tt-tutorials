...
cannotDie = if 1 + 1 == 3 then die "horrible death" else ()
...
result n d | isPositive d = "Result = " ++ show (n `divide` d)
           | otherwise    = "Humph, please enter positive denominator!"

{-@ isPositive :: (Num a, Ord a) => x : a -> {v : Bool | v <=> x > 0} @-}
-- isPositive :: Int -> Bool
isPositive x = x > 0
